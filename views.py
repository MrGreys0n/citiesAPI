...
async def add_city(request):
    try:
        ...
        existing_city = await storage.get_city(city_name)
        if existing_city is not None:
            return web.json_response(
                {'error': f'City with name {city_name} already exists {existing_city}'}, status=400
            )
...
async def delete_city(request):
    ...
    if existing_city is None:
        return web.json_response({'error': 'There is no such city in database'}, status=400)
